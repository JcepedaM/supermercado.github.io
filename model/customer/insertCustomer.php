		...
		if(isset($fullName) && isset($mobile) && isset($address)) {
			...
			if(!empty($phone2)){
				if(filter_var($phone2, FILTER_VALIDATE_INT) === false) {
					...
					echo '<div class="alert alert-danger"><button type="button" class="close" data-dismiss="alert">&times;</button>Por favor introduce un número de móvil válido 2</div>';
					exit();
				}
			}
			...
			if($address == ''){
				// Address 1 is empty
				echo '<div class="alert alert-danger"><button type="button" class="close" data-dismiss="alert">&times;</button>Por favor ingrese la dirección 1</div>';
				exit();
			}
			...
			if($fullName == ''){
				// Full Name is empty
				echo '<div class="alert alert-danger"><button type="button" class="close" data-dismiss="alert">&times;</button>Por favor ingrese el nombre completo.</div>';
				exit();
			}
