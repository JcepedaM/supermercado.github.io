		...
		if(!empty($itemImageItemNumber)){
			...
			if($_FILES['itemImageFile']['name'] != ''){
				...
				if($itemNumberStatement->rowCount() > 0){
					...
					echo '<div class="alert alert-danger"><button type="button" class="close" data-dismiss="alert">&times;</button>No se permite el tipo de imagen. Por favor seleccione una imagen válida.</div>';
					exit();
					}
				}
