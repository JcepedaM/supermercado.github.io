<?php
	...
	if(isset($_POST['loginUsername'])){
		...
		if(!empty($loginUsername) && !empty($loginUsername)){
			...
			if($loginUsername == ''){
				echo '<div class="alert alert-danger"><button type="button" class="close" data-dismiss="alert">&times;</button>Por favor ingrese el nombre de usuario</div>';
				exit();
			}
			
			// Check if password is empty
			if($loginPassword == ''){
				echo '<div class="alert alert-danger"><button type="button" class="close" data-dismiss="alert">&times;</button>Por favor, ingrese contraseña</div>';
				exit();
			}
			...
			// Check the given credentials
				...
				if ($checkUserStatement->rowCount() > 0) {
					...
					// Asigna el valor de userID a la variable de sesión $_SESSION['userID']
			...
			echo '<div class="alert alert-danger"><button type="button" class="close" data-dismiss="alert">&times;</button>Por favor ingrese Nombre de Usuario y Contraseña</div>';
			exit();
		}
	}
?>